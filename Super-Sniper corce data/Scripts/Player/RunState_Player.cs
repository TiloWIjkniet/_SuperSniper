...
public class RunState_Player : BaseState_Player
{
   ...
   public IEnumerator col(Brains_Player brains)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            brains.image[0].color = Color.Lerp(brains.image[0].color, new Color (255,255,255,0.5f), (float)i/5);
            yield return new WaitForSeconds(0.05f);
        }   
        brains.image[0].color =  new Color (255,255,255,0.5f);       
    }
    ...
    public override void UpdateState(Brains_Player brains)
    {
        ...
        if(brains.generalUsage_Player.fixedSpeed < 6) return;
         brains.currentStamina -=  Time.deltaTime * 0.5f;
